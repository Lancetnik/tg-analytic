...
class TgClientRepository:
    ...
    async def create(cls):
        ...
        for user in await get_users_with_accounts():
            ...
            for account in user.accounts:
                ...
                accounts[account.id] = await start_client(
                    account.login, account.api_id, account.api_hash
                )
    ...
    async def delete_account(self, user_id: int, account_id: int):
        ...
        if user.get(DEFAULT) == account_id:
            user[DEFAULT] = None
        client = user.get(CLIENTS, {}).pop(account_id)
        await client.disconnect()
        self._clients[user_id] = user


    async def create_account(self, account: TgUserAccount):
